...
class RealNumberGrammar(
		val input: ParserInput,
		functions: Map[String, Seq[Double] => Double] = RealNumberGrammar.defaultFunctionMap
) extends Parser {

	protected def realNumberLine = rule {
		expression ~ EOI
	}

	protected def expression: Rule1[Double] = rule {
		term ~ zeroOrMore(
			'+' ~ term ~> ((_: Double) + _)
					| (ch('-') | '−') ~ term ~> ((_: Double) - _)
		)
	...
	protected def term: Rule1[Double] = rule {
		factor ~ zeroOrMore(
	...
	/**
	  ...
	  */
	protected def function: Rule1[Double] = rule {
	...
	protected def parameterList: Rule1[Seq[Double]] = rule {
		'(' ~ oneOrMore(expression).separatedBy(",") ~ ')'
	}

	protected def parentheses = rule {
		'(' ~ expression ~ ')'
	}

	protected def number: Rule1[Double] = rule {
		capture(optional('-') ~ (expNotation | floatingPoint | integer | inf | nan)) ~> ((s: String) => s.toDouble)
	}

	protected def expNotation: Rule0 = rule {
	...
	protected def floatingPoint: Rule0 = rule {
		oneOrMore(CharPredicate.Digit) ~ ch('.') ~ oneOrMore(CharPredicate.Digit)
	}

	protected def fraction: Rule0 = rule {
		ch('.') ~ oneOrMore(CharPredicate.Digit)
	}

	protected def nan: Rule0 = rule { "NaN" }
	protected def inf: Rule0 = rule { "Infinity" }
}