...
import kokellab.gale.utils.{SiPrefix, Squinter}
...
class TimeRangeGrammar(converter: Time => Long, units: Set[String], siPrefixes: Set[String], defaultUnit: Option[String]) {
	...
	protected lazy val squinter: Squinter[Time] = new Squinter(
		Time.apply(_),
		units,
	...
	protected def eval(expression: String): (Long, Long) = {
...
//	private val timePattern = """((?:[\$A-Za-z_0-9]|(?:\d+(?:E[\+\-]?\d+)?)) *(?:[mk]?[smh])?)"""
	protected val timePattern = """((?:[\$A-Za-z_0-9]|(?:\d+(?:E[\+\-]?\d+)?)) *(?:[""" + siPrefixes.mkString + "]?[" + units.mkString + "])?)"
	protected val rangePattern = (timePattern + """ *[-–] *""" + timePattern).r
