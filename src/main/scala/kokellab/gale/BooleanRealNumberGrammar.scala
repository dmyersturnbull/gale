...
class BooleanRealNumberGrammar(
		override val input: ParserInput, tolerance: Double = BooleanRealNumberGrammar.DEFAULT_TOLERANCE,
	...
	protected def booleanLine = rule { booleanExpression ~ EOI }

	protected def booleanExpression = rule {
	...
	protected def wrappedJunction: Rule1[Boolean] = rule {
		('(' ~ booleanExpression ~ ')') | junction
	}

	protected def junction: Rule1[Boolean] = rule {
	...
	protected def condition: Rule1[Condition] = rule {
	...
	/**
	  ...
	  */
	protected case class Condition(rhs: Double, operation: Double => Boolean)
}
