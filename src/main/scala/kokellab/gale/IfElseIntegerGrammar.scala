...
class IfElseIntegerGrammar(override val input: ParserInput, override val functions: Map[String, Seq[Int] => Int]) extends BooleanIntegerGrammar(input, functions) {
	...
	protected def ifElseLine: Rule1[Option[Int]] = rule { ifElse ~ EOI }

	protected def ifElse: Rule1[Option[Int]] = rule { someExpression | ifElifElse }

	protected def ifElifElse: Rule1[Option[Int]] = rule {
		ifElif ~ elseExpr ~> ((a: Option[Int], b: Option[Int]) => if (a.isDefined) a else if (b.isDefined) b else None)
	}

	protected def elseExpr: Rule1[Option[Int]] = rule {
		optional("else:" ~ ifElse) ~> ((value: Option[Option[Int]]) => if (value.isDefined) value.get else None)
	}

	protected def ifElif: Rule1[Option[Int]] = rule {
	...
	/**
	  ...
	  */
	protected def elifs: Rule1[Option[Int]] = rule {
	...
	/**
	  ...
	  */
	protected def someExpression: Rule1[Option[Int]] = rule { expression ~> ((d: Int) => Some(d)) }

	protected def conditionRule: Rule1[Option[Int]] = rule {
