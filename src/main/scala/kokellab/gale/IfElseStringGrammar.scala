...
class IfElseStringGrammar(override val input: ParserInput, override val functions: Map[String, Seq[Int] => Int]) extends BooleanIntegerGrammar(input, functions) {
	...
	protected def ifElseLine: Rule1[Option[String]] = rule { ifElse ~ EOI }

	protected def ifElse: Rule1[Option[String]] = rule { someValue | ifElifElse }

	protected def ifElifElse: Rule1[Option[String]] = rule {
		ifElif ~ elseExpr ~> ((a: Option[String], b: Option[String]) => if (a.isDefined) a else if (b.isDefined) b else None)
	}

	protected def elseExpr: Rule1[Option[String]] = rule {
		optional("else:" ~ ifElse) ~> ((value: Option[Option[String]]) => if (value.isDefined) value.get else None)
	}

	protected def ifElif: Rule1[Option[String]] = rule {
	...
	/**
	  ...
	  */
	protected def elifs: Rule1[Option[String]] = rule {
	...
	/**
	  ...
	  */
	protected def someValue: Rule1[Option[String]] = rule { value ~> ((s: String) => Some(s)) }

	protected def value: Rule1[String] = rule { '"' ~ capture(zeroOrMore(noneOf("\""))) ~ '"'}

	protected def conditionRule: Rule1[Option[String]] = rule {
