...
object TimeSeriesGrammar {
	...
	private val mainPattern = """ *(.*?) *(?: *@ *(\-?\d+(?:\.\d+)?))?""".r

	protected def extract(expression: String): (String, Int) = expression match {
...
class TimeSeriesGrammar[@specialized(Float, Double, Byte, Short, Int) A : ClassTag](
	...
	protected def arrayAccessReplacer(values: Array[A])(m: Match): String = {
	...
	private def fullReplacer(values: Array[A]): Int => String = {
