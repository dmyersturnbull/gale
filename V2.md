...
It was designed for users with some math but little or no programming knowledge. There are no loops, variables, or scope. It's purely functional and uses type inference. The unique features are: implicit sliding over matrix indices, implicit recursion, and dimensionality constraints as part of the typing. With few exceptions, operations can't fail during execution: Matrix multiplication isn't permitted unless the input dimensionality constraints guarantee it's safe, and divide-by-zero results propagate through. (The parser will take your word that a matrix is invertible, however.)
